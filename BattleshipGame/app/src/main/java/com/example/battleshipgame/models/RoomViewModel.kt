...
class RoomViewModel : ViewModel() {
    ...
    fun updateStatistics(isWinner: Boolean, userId: String) {
        ...
        val childUpdates = hashMapOf<String, Any>(
            "user_statistics/${userId}/${date}" to postValues
        )
    ...
    fun updateStepOwner(owner: String) {
        Firebase.database.reference.child("rooms").child(this.roomId.toString())
            .child("step_owner").setValue(owner)
    }

    fun updateUserStatus(status: String) {
        Firebase.database.reference.child("rooms").child(roomId.toString())
            .child(USER_TAG.toString())
